...
class BaseTrainer:
    ...
    def _train_process(self):
        ...
        for epoch in range(self.start_epoch, self.epochs + 1):
            ...
            #         metric = logs.get("dev_loss", logs.get("eval_loss"))
        ...
        with torch.no_grad():
            ...
            for met in self.metrics["inference"]:
                if hasattr(met, "compute"):
                    value = met.compute()
                    self.evaluation_metrics.update(met.name, value)
                    if self.writer is not None:
                        self.writer.add_scalar(f"{part}/{met.name}", value)
    ...
    def _save_checkpoint(self, epoch, save_best=False, only_best=False):
        ...
        if save_best:
            best_path = str(self.checkpoint_dir / f"model_best{epoch}.pth")
            torch.save(state, best_path)
            if self.config.writer.log_checkpoints:
                self.writer.add_checkpoint(best_path, str(self.checkpoint_dir.parent))
            self.logger.info(f"Saving current best: model_best{epoch}.pth ...")
